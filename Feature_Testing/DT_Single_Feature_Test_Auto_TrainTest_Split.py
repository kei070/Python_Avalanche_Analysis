...
out_path = f"/PATH_TO_ACCURACY/Single_Feature/Between{h_low}_and_{h_hi}m/{region}/"
...
fid_test_d_m = {"feature":all_feat, "far":[], "mfr":[]}
fid_test_all_d_m = {"feature":all_feat, "far":[], "mfr":[]}
fid_train_d_m = {"feature":all_feat, "far":[], "mfr":[]}
fid_test_d_s = {"feature":all_feat, "far":[], "mfr":[]}
fid_test_all_d_s = {"feature":all_feat, "far":[], "mfr":[]}
fid_train_d_s = {"feature":all_feat, "far":[], "mfr":[]}

for sel_feat in all_feat:
    fid_test_d = {"feature":all_feat, "rpc":[], "tss":[], "far":[], "pod":[], "pon":[], "hss":[], "pss":[], "mfr":[]}
    fid_test_all_d = {"feature":all_feat, "rpc":[], "tss":[], "far":[], "pod":[], "pon":[], "hss":[], "pss":[],
                      "mfr":[]}
    fid_train_d = {"feature":all_feat, "rpc":[], "tss":[], "far":[], "pod":[], "pon":[], "hss":[], "pss":[], "mfr":[]}
    ...
    for shuffles in np.arange(nshuffle):
        ...
        odata_x = pd.concat([train_df[sel_feat], test_df[sel_feat]])
        odata_y = pd.concat([train_df[f"{train_s.lower()}_y_balanced"], test_df[f"{test_s.lower()}_y_balanced"]])

        odata_x_all = pd.concat([train_all_df[sel_feat], test_all_df[sel_feat]])
        ...
        train_x_all = np.array(train_x_all).reshape(-1, 1)
        test_x = np.array(test_x).reshape(-1, 1)
        test_x_all = np.array(test_x_all).reshape(-1, 1)
...
out_df = pd.DataFrame({"feature":all_feat, "train":acc_train_m, "test":acc_test_m, "test_all":acc_test_all_m,
                       "train_std":acc_train_s, "test_std":acc_test_s, "test_all_std":acc_test_all_s})

out_df.to_csv(out_path + "DT_Single_Feature_Accuracy.csv")


#%% also store the other model metrics
fid_train_m.to_csv(out_path + "DT_Single_Feature_Metrics_Train.csv")
fid_test_m.to_csv(out_path + "DT_Single_Feature_Metrics_Test.csv")
fid_test_all_m.to_csv(out_path + "DT_Single_Feature_Metrics_Test_All.csv")
fid_train_s.to_csv(out_path + "DT_Single_Feature_Metrics_Std_Train.csv")
fid_test_s.to_csv(out_path + "DT_Single_Feature_Metrics_Std_Test.csv")
fid_test_all_s.to_csv(out_path + "DT_Single_Feature_Metrics_Std_Test_All.csv")
